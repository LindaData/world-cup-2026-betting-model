...
def build_steps(args: argparse.Namespace, python_exe: str, rscript_exe: str) -> list[Step]:
    ...
    if not args.skip_model:
        ...
        steps.append(Step("Fit expanded model suite", [rscript_exe, "R/21_fit_expanded_model_suite.R"]))
        steps.append(Step("Build expanded feature population", [rscript_exe, "R/22_build_expanded_feature_population.R"]))
        steps.append(Step("Fit population expansion models", [rscript_exe, "R/23_fit_population_expansion_models.R"]))
        steps.append(Step("Run distribution diagnostics", [rscript_exe, "R/24_distribution_diagnostics.R"]))
        ...
        steps.append(Step("Build local recording registry", [rscript_exe, "R/26_build_local_recording_registry.R"]))
        steps.append(Step("Build game archive", [rscript_exe, "R/25_build_game_archive.R"]))
        steps.append(
